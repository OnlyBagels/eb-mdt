...
lib.callback.register('mdt:searchProfiles', function(source, searchQuery)
    ...
    if searchQuery and searchQuery ~= '' then
        ...
        -- Search using metadata JSON fields directly
        local query = [[
            SELECT 
                id,
                citizenid,
            ...
            WHERE 
                ...
                LOWER(TRIM(REPLACE(JSON_EXTRACT(charinfo, '$.firstname'), '"', ''))) LIKE ? OR 
                LOWER(TRIM(REPLACE(JSON_EXTRACT(charinfo, '$.lastname'), '"', ''))) LIKE ? OR
                LOWER(CONCAT(
                    TRIM(REPLACE(JSON_EXTRACT(charinfo, '$.firstname'), '"', '')), 
                    ' ', 
                    TRIM(REPLACE(JSON_EXTRACT(charinfo, '$.lastname'), '"', ''))
                )) LIKE ? OR
                LOWER(CONCAT(
                    TRIM(REPLACE(JSON_EXTRACT(charinfo, '$.lastname'), '"', '')), 
                    ' ', 
                ...
                LOWER(TRIM(REPLACE(JSON_EXTRACT(metadata, '$.bloodtype'), '"', ''))) LIKE ? OR
                LOWER(TRIM(REPLACE(JSON_EXTRACT(metadata, '$.fingerprint'), '"', ''))) LIKE ?
            LIMIT 50
        ]]
        
        local results = exports.oxmysql:executeSync(query, {
            searchPattern,  -- name
            searchPattern,  -- citizenid
            searchPattern,  -- firstname
            searchPattern,  -- lastname
            searchPattern,  -- firstname + lastname
            searchPattern,  -- lastname + firstname
            searchPattern,  -- bloodtype from metadata
            searchPattern   -- fingerprint from metadata
        })
        
        if results then
            -- First pass: Add all matching results
            ...
            -- Second pass: Check if search query is a DNA code (hex string)
            -- DNA codes are hex strings, typically 14-32 characters long
            if #searchLower >= 10 and searchLower:match("^%x+$") then
                -- Get ALL citizenids and check their DNA hash
                local allCitizens = exports.oxmysql:executeSync([[
                    SELECT 
                        id,
                        citizenid,
                        license,
                ...
                for _, citizen in ipairs(allCitizens) do
                    local dnaCode = DnaHash(citizen.citizenid)
                    
                    -- Check if DNA code matches search
                    if string.find(string.lower(dnaCode), searchLower, 1, true) then
                        -- Check if we already added this citizen
                        local alreadyAdded = false
                        for _, profile in ipairs(profiles) do
                            if profile.citizenid == citizen.citizenid then
                                alreadyAdded = true
                                break
                            end
                        end
...
lib.callback.register('mdt:getProfile', function(source, citizenid)
    ...
    -- Get biometric data from metadata
    local fingerprint = metadata.fingerprint or metadata.finger or 'Not on file'
    local bloodType = metadata.bloodtype or metadata.blood_type or 'Unknown'
    local bloodLevel = metadata.blood or 100
    local dnaCode = DnaHash(citizenid) -- Generate DNA code from citizenid
    ...
    return {
        ...
        bloodType = bloodType,
        blood = bloodLevel,
        dnaCode = dnaCode, -- Add DNA code to profile
...
lib.callback.register('mdt:addLicense', function(source, data)
    ...
    -- Update using QBOX export
    exports.qbx_core:SetMetadata(data.citizenid, 'licences', metadata.licences)
...
-- Export to get DNA code from citizenid
exports('GetDnaCode', function(citizenid)
    return DnaHash(citizenid)
end)
...
RegisterCommand('mdtdebug', function(source)
    ...
    if player then
        ...
        print('Citizen ID:', player.PlayerData.citizenid)
        print('DNA Code:', DnaHash(player.PlayerData.citizenid))
        print('^2=====================^0')
        ...
        TriggerClientEvent('chat:addMessage', src, {
            color = {255, 255, 255},
            multiline = true,
            args = {"MDT Debug", string.format("Job: %s | Grade: %d | On Duty: %s | Can Manage Licenses: %s | DNA: %s", 
                player.PlayerData.job.name, 
                player.PlayerData.job.grade.level or 0, 
                tostring(player.PlayerData.job.onduty),
                tostring(CanManageLicenses(player)),
                DnaHash(player.PlayerData.citizenid)
            )}
        })
    end
end, false)
