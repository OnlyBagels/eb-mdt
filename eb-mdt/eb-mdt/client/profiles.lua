...
-- Get citizen charges history - FIXED
RegisterNuiCallback('getCitizenCharges', function(data, cb)
    lib.callback('mdt:server:getCitizenCharges', false, function(result)
        -- Return the charges array directly, not the result object
        if result and result.success then
            cb(result.charges or {})
        else
            cb({})
        end
    end, data.citizenid)
