...
const ProfilesPage: React.FC<ProfilesPageProps> = ({
  ...
  // Criminal charges states - FIXED
  const [citizenCharges, setCitizenCharges] = useState<CitizenCharge[]>([]);
  ...
  const viewProfile = async (citizenid: string) => {
    ...
    try {
      ...
      if (profile) {
        ...
        // Load criminal charges - FIXED
        try {
          setChargesLoading(true);
          const charges = await fetchNui<CitizenCharge[]>("getCitizenCharges", {
            citizenid,
          });
          setCitizenCharges(charges || []);
        } catch (error) {
  ...
  const handleBackToList = useCallback(() => {
    ...
    setCitizenCharges([]); // FIXED - Clear charges
  }, []);
          ...
          <div className="flex items-center gap-4">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Search by name, ID, blood, fingerprint..."
                value={searchQuery}
            ...
            {activeTab === "criminal" && (
              <div className="space-y-4">
                ...
                <div className="bg-mdt-accent p-4 rounded-lg">
                  ...
                  {chargesLoading ? (
                  ...
                  ) : citizenCharges.length > 0 ? (
                    <div className="space-y-2 max-h-96 overflow-auto custom-scrollbar">
                      {citizenCharges.map((charge) => (
                        <div
                          key={charge.id}
                          className="bg-mdt-bg p-3 rounded-lg border border-mdt-border"
                        >
                          <div className="flex items-start justify-between mb-2">
                            <div className="flex-1">
                              ...
                              {charge.report_number && (
                                <div className="text-xs text-primary-200">
                                  Report: {charge.report_number}
                                  {charge.report_title && ` - ${charge.report_title}`}
                                </div>
                              )}
                            </div>
                            <div className="text-xs text-primary-200 whitespace-nowrap">
                              {new Date(charge.applied_at).toLocaleDateString()}
                            </div>
                          </div>
                          <div className="flex gap-3 text-xs">
                            {charge.fine > 0 && (
                              <div className="flex items-center gap-1">
                                ...
                                <span className="text-white">${charge.fine.toLocaleString()}</span>
                              </div>
                            )}
                            {charge.months > 0 && (
                              <div className="flex items-center gap-1">
                                <i className="fas fa-clock text-orange-400"></i>
                                <span className="text-white">{charge.months} months</span>
                              </div>
                            )}
                          </div>
                        </div>
                    ...
                    <div className="text-center py-8">
                      <i className="fas fa-gavel text-4xl text-primary-400 mb-2"></i>
                      <p className="text-primary-200 text-sm">No charges on record</p>
                    </div>
                  )}
                </div>
              </div>
