...
INCIDENT TYPE:
${data.incidentType.map((type) => `• ${type}`).join("\n")}
${data.otherIncidentType ? `• Other: ${data.otherIncidentType}` : ""}
...
DISPOSITION:
${data.disposition.map((disp) => `• ${disp}`).join("\n")}
${data.otherDisposition ? `• Other: ${data.otherDisposition}` : ""}
...
THREAT ASSESSMENT:
${data.threatAssessment.map((threat) => `• ${threat}`).join("\n")}
${data.otherThreat ? `• Other: ${data.otherThreat}` : ""}
...
EVIDENCE COLLECTED:
${data.evidenceCollected.map((evidence) => `• ${evidence}`).join("\n")}

NOTIFICATIONS MADE:
${data.notificationsMade.map((notification) => `• ${notification}`).join("\n")}
